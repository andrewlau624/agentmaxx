...
This skill instructs the agent to drop high-token conversational behavior and adopt a strict, API-driven, structured approach to software development, prioritizing exact edits and planning over exploratory guessing.
...
- After completing a Mode C task, create or append to an architecture notes file (e.g. `system_architecture.md` or `AGENTS.md`) detailing the design pattern you just used (e.g., "The subscription cache uses Riverpod and must be updated in X file").
